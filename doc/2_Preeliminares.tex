...
centralidad con las cuales se determinarán los datos más importantes
...
contenido y las relaciones entre los datos publicados.
Con esto se logra mejorar la interoperatividad de internet pues los
...
estándares de representación como son XML, XML Schema, RDF, RDF Schema, OWL y
SPARQL.
Estas tecnologías se utilizan actualmente para generar datos enlazados
(\emph{Linked Data}), que buscan enlazar información arbitraria en la web
generando así una ``\emph{red de las cosas del mundo, descrita por los
...
\subsection{URI}
Una URI (del las siglas del íngles \emph{Uniform Resource Identifier}) es una
cadena de caracteres utilizada para identificar un recurso inequívocamente.
Si bien en un principio solo acepta un subconjunto de caracteres ASCII (el
alfabeto en íngles y los números) y ``código porciento'' (en el cual, por
ejemplo '\%26' es '\&') el estándar fue extendido el 2005 a IRI
(\emph{Internationalized Resource Identifier}) que puede contener caracteres
Unicode/ISO 10646, incluyendo chino, japonés, koreano, entre
otros\cite{gangemi2006bourne}.
...
\begin{itemize}
  ...
  \item
    Las dos barras (\tt{//}) son requeridas para algunos esquemas. Cuando la
  ...
  \item
    ...
    \begin{itemize}
      \item
        ...
        en un arroba (\tt{@}).
      \item
        Un anfitrión (\emph{host}) que puede ser un nombre registrado de una
        dirección IP.
      \item
  ...
  \item
    ...
    interrogación (\tt{?}) y, a pesar de no tener una sintaxis claramente
...
\subsection{XML}
XML es el acrónimo del íngles \emph{Extensible Markup Language}. Es un
...
En la figura~\ref{fig:xmlex} vemos la libertad que da XML a la hora de
...
contenida en éste puede ser sensible a la aplicación y al lenguaje definido con
...
\subsection{XSD}
XSD (del íngles \emph{XML Schema Definition}) es una recomendación del W3C que
...
\begin{figure}[htpb]
  ...
  \vspace{-.2cm}
  \caption*{\small
    Definiendo una película con XSD. 
    Como podemos notar, se genera un archivo XML y se agrega el prefijo XSD
    \url{http://www.w3.org/2001/XMLSchema}, el resto es solo asignar el tipo de
    dato requerido.
  }
\end{figure}
...
Además, un conjunto de triples RDF será representado naturalmente por un grafo
dirigido (con $s$ y $p$ como nodos y $o$ como el arco que los une).
...
codi-ficación de un triple varía dependiendo del tipo de archivo en el que se
...
\begin{itemize}
  ...
  \item 
    Si una línea termina en punto y coma (\tt{;}) la siguiente línea mantiene el
...
mismo ejemplo escrito en XML/RDF (figura \ref{fig:triples:rdf}) y como éstos
...
\begin{figure}[htpb]
  ...
  \vspace{-.2cm}
  \caption*{\small
    Tanto \ref{fig:triples:ttl} como \ref{fig:triples:rdf} presentan la misma
    información, pero en formatos distintos. Turtle es un formato fácil de leer
    tanto por humanos como por computadoras mientras que XML/RDF, si bien es más
    difícil de leer por los humanos, es el estándar para la transmisión de
    metadatos por Internet. 

    Por su parte, el grafo de~\ref{fig:triples:grafo} es la representación más
    natural de las relaciones descritas.
  }
\end{figure}
...
más avanzada y por ello es la base de otros vocabularios como OWL y SKOS.
...
\end{enumerate}
Así, toda ontología o conclusión válida en OWL Lite tambien lo será en OWL DL y
éstas a su vez lo serán en OWL Full.
...
\subsection{SPARQL}
SPARQL (del íngles \emph{SPARQL Protocol and RDF Query Language}) es un lenguaje
...
En~\cite{perez2006semantics} por su parte se estudia la complejidad de la
evaluación para varios aspectos del lenguaje.
Se presenta, mediante lógica de conjuntos, operaciones básicas por las cuales se
puede resolver de manera eficiente la evaluación de una consulta, gracias a la
conmutatividad y asociatividad de los operandos.
Además se evidencia la problemática de la obtención del mínimo conjunto de datos
y la transformación de operaciones como \tt{OPTIONAL}, \tt{FILTER}, \tt{MINUS},
entre otras, en una combinación de las básicas.
...
\begin{figure}[htpb]
  ...
  \vspace{-.2cm}
  \caption*{\small
    Ambas consultas hechas a la DBpedia sobre películas, sus actores, directores
    y su relación.
  }
\end{figure}
...
pero los resultados serán triples RDF con las relaciones especificadas.
En ellas solo se guarda la fuente de la información como la cadena ``dbpedia''
y al actor y director como participantes de la película.

Podemos notar cómo la consulta \tt{CONSTRUCT} posee la capacidad de generar
nueva información con base en los resultados de una búsqueda y no se limita
solamente a las relaciones ya existentes, gracias a ello se pueden formar
triples RDF relativamente simples de consultas mucho más complejas.
Por otro lado, si solo se busca retornar la información en formato RDF no es
necesario que la claúsula tenga argumentos (\tt{CONSTRUCT WHERE \{...\}} ),
aunque este método no funcionará con consultas más complejas como aquellas con
triples opcionales.
...
la versión que se mantiene en línea.
Versiones más recientes pueden ser descargadas directamente de la página Web.
...
\begin{figure}[htpb]
  ...
  \vspace{-.2cm}
  \caption*{
    Extraído de \it{Link Analysis of Life Science Linked Data}\cite{hu2015link}.
    Las flechas representan las relaciones existentes entre las distintas bases
    de datos que incorpora el proyecto.
  }
...
presenta y el tipo de datos que maneja.

Según su página Web\footnote{\url{http://www.drugbank.ca/}}, DrugBank es una base de datos única
...
\section{Grafos y centralidad}\label{ea:cent}
Como se explicó en la sección~\ref{sw:rdf}, un conjunto de triples RDF puede ser
representado por un grafo dirigido, gracias a ello es que podemos obtener
métricas de éstos y determinar diferentes propiedades y características
asociadas. En el contexto de este documento se trabajará específicamente con la 
centralidad.

En la sección~\ref{gc:gr}, se introduce brevemente el concepto de grafo junto a
algunas de sus características y propiedades básicas.
Mientras que en la sección\ref{gc:c} hablaremos de la centralidad y explicaremos
las formas más populares de calcular esta métrica.

\subsection{Grafos}\label{gc:gr}
...
adecuado. Un grafo dirigido (o digrafo) es aquel en el cual los arcos son pares
...
Del ejemplo mostrado en la figura~\ref{fig:exgraph} es posible destacar lo
siguiente:
\begin{itemize}
  \item
...
\subsection{Tipos de centralidad}\label{gc:c}
...
dominio del grafo con el que estemos trabajando.
...
Desde su definición no existe una noción única para determinar qué es realmente 
...
Bajo las medidas de \bf{centralidad de grado} y \bf{cercanía}, se le da mayor
importancia a los nodos y partes del grafo más conectadas con el resto del
mismo.
Por su parte, la \bf{intermediación} determina los puntos críticos por donde
circula la información de manera que si se extraen estos nodos se pierde más
conectividad en general.
Por último, la \bf{centralidad de vector propio} determina cuáles son los nodos
más importantes para mantener el comportamiento y la forma de una red.
...
al nodo más central o a la suma de la centralidad de todos los nodos del grafo.

\subsubsection{Centralidad de grado}\label{ea:cent:degree}
La centralidad de grado (en íngles \emph{degree centrality}) es la primera y más
...
La cercanía (en íngles \emph{closeness centrality}) es una medida de centralidad
...
nodos a un nodo en particular.

Digamos $d(V_i, V_j)$ como la distancia del camino más corto entre
$V_i$ y $V_j$ ($\infty$ si éste no existe), la cercanía queda definida como sigue:
\begin{equation}
  \label{eq:far}
...
\bf{lejanía}. Este cálculo se hace inadecuado para grafos no fuertemente
...
de cercanía más natural que consiste en calcular el recíproco de la anterior: 
\begin{equation}
  \label{eq:clo}
...
valores mayores indican mayor centralidad. Se muestra además una normalización
...
La intermediación (del íngles \emph{betweenness centrality}) cuantifica la
...
social.
Bajo esta definición se podría decir que una persona localizada
estratégicamente en el ``centro'' de la comunicación de un grupo, tiene control
sobre la misma y puede distorsionarla.
Empíricamente esto se transforma en características como el liderazgo, la moral,
la satisfacción personal, la velocidad de respuesta, la frecuencia de error y la
...
Además, para el cálculo de la intermediación es necesario contar el número de
...
Este proceso, al igual que para el cálculo de la cercanía, implica la obtención
...
Será particularmente útil en este trabajo el algoritmo de Brandes ya que
podemos representar un conjunto de triples RDF como un grafo dirigido sin pesos
en sus arcos.
La idea principal es realizar una búsqueda en anchura para cada nodo e ir
incrementando el número de caminos más cortos que pasan por todos los nodos
visitados.
Definamos $\delta_{st}(v)$ como $\sigma_{st}(v)/\sigma_{st}$, luego,
la segunda idea principal es que $\delta_{st}(v)$ puede ser obtenido por
separado.
Digamos la \it{dependencia} de un nodo fuente $V_s \in V$ a un nodo $V_v \in
...
\begin{equation}\label{eq:brand}
  ...
  (1 + \delta_s(w))
\end{equation}
Así, el cálculo de la intermediación para cada nodo se realiza en dos partes,
primero obtenemos la distancia y el camino más corto a todos los demás nodos y
luego, desde el final hacia el principio, se acumula la dependencia según la
ecuación~\ref{eq:brand}.

\subsubsection{Centralidad de vector propio}
La centralidad de vector propio (en íngles \emph{Eigenvector centrality}) fue
...
los resultados mostrados en la tabla~\ref{tab:exeigen}. \begin{table}[h]
  \centering
  \begin{tabular}{|l|c|c|c|c|c|c|}
    \hline         &  A  &  B  &  C  &  D  &  E  &  F  \\\hline
...
Como podemos notar, esta centralidad no es adecuada para grafos dirigidos tan
pequeños como el presentado en el ejemplo, pues su matriz de adyacencia es
singular, es decir tiene determinante nulo.

\subsubsection{Comparación}
Para evidenciar la diferencia entre los resultados del cálculo de las
centralidades enunciadas en este capítulo, se presenta la figura~\ref{fig:exoth}
a modo de ejemplo.
...
\begin{figure}[htpb]
  ...
  \begin{tikzpicture}
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Grafo con diferentes centralidades.} \label{fig:exoth}
\end{figure}

En la tabla~\ref{tab:exoth} se muestran los resultados del cálculo de las
centralidades. Para facilitar la comparación se presentan valores relativos a la
centralidad máxima.

La simetría del grafo genera resultados simétricos.
Como podemos notar, el nodo más central no cambia para ninguna centralidad (para
grafos más grandes y no simétricos esto no siempre ocurre).

Se puede apreciar como la intermediación presenta los resultados más diferentes,
pues, todo aquel nodo que no sea esencial para la comunicación del grafo, tiene
valor cero.
Es interesante ver que el nodo C en los demás resultados es el segundo más
central (en especial en la centralidad de valor propio), pero aún así no es un
punto crítico en la comunicación.

\begin{figure}[htpb]
    \centering
    \begin{tabular}{|l|c|c|c|c|c|c|c|} \hline
                           &  A   & B &  C   & D & E &  F   &  G   \\\hline
      Centralidad de Grado & 0,25 & 1 & 0,75 & 1 & 1 & 0,25 & 0,25 \\\hline
      Cercanía             & 0.61 & 1 & 0,88 & 1 & 1 & 0.61 & 0.61 \\\hline
      Intermediación       &    0 & 1 &    0 & 1 & 1 &    0 &    0 \\\hline
      Centralidad de Vector Propio 
                           & 0.30 & 1 & 0,92 & 1 & 1 & 0.30 & 0.30 \\\hline
    \end{tabular}
    \caption{Resultados de centralidad para la figura~\ref{fig:exoth}.}
    \label{tab:exoth}
\end{figure}
