...
\begin{figure}[htpb]
  ...
  \begin{subfigure}[b]{\textwidth}
    ...
    \end{tabular}
    \caption{Ejemplo de consulta tipo SELECT.}
  ...
  \begin{subfigure}[b]{\textwidth}
    ...
    \end{tabular}
    \caption{Ejemplo de consulta tipo CONSTRUCT.}
...
base de dato como un nodo del grafo y sus arcos están determinados
por la cantidad de enlaces entre sus respectivas entidades.
La tabla \ref{tab:bio2RDFdataset} por su parte muestra el número de entidades
únicas y relaciones en cada base de datos.

Como podemos notar DrugBank posee una alta importancia bajo ambos esquemas.
...
\begin{itemize}
  ...
  \item
    La simetría y transitividad no logran traspasar las fronteras del propio
...
%TODO: Agregar referencias y arreglar redacción.
El concepto de centralidad hace referencia a la medida de importancia relativa
de un nodo en un grafo. Desde su definición no existe una noción única para
determinar cual es la importancia de un nodo y por ello han florecido diferentes
técnicas y métodos para ellos. Para entender el concepto es importante recordar
algunos conceptos básicos de teoría de grafos.
...
sea el camino que existe entre las ciudades. También podemos representar una red
social donde cada persona sea un nodo y sus relaciones con los demás individuos
estén inscritas en los arcos.

%TODO: redo this
Digamos $G=(V, E)$ un grafo donde $V$ es un conjunto de nodos y $E$ es un
conjunto de arcos. Podemos denotar cada nodo de $G$ como $v \in V$ y cada arco
como $e \in E: e = (v, w), v,w \in V$

\subsection{Tipos de centralidad}
\subsubsection{La centralidad de grado}
\subsubsection{La cercanía}
\subsubsection{La intermediación}
\subsubsection{La centralidad de vector propio}
